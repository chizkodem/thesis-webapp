...
document.getElementById("submit").addEventListener("click", async function (e) {
  ...
  try {
    ...
    window.location.href = "webappaccmanager/admincrudmain.html";
...
async function signUpUser(email, password, fullName) {
  ...
  await set(ref(db, "user/" + updatedEmail), {
    fullName: fullName,
    email: email,
  });
...
function saveCurrentUserToSessionStorage(user, fullName) {
  const userJSON = JSON.stringify({
    uid: user.uid,
    email: user.email,
    fullName: fullName, // Store the fullName in session storage
  });
